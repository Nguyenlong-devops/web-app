...
def init_db():
    ...
    steps = [
        ...
        # Migration: nhớ lại domain nào bắt buộc LDAPS (636, DC mới như 2025/2026 hoặc DC cũ đã
        # bật signing bắt buộc) — để các lần login SAU không phải mất thời gian thử LDAP thường
        # (389) trước rồi mới fallback, mà vào thẳng LDAPS luôn.
        """ALTER TABLE domain_config ADD COLUMN IF NOT EXISTS use_ldaps BOOLEAN NOT NULL DEFAULT FALSE""",
    ]
...
def get_active_domain_config():
    ...
    try:
        cur.execute("""
            SELECT id, ldap_host, domain_suffix, base_dn, admin_group_dn, ssh_host, ssh_user, ssh_pass, use_ldaps
        ...
        return {
            ...
            "ssh_host": row[5], "ssh_user": row[6], "ssh_pass": row[7], "use_ldaps": row[8],
        }
    except Exception as e:
    ...
    try:
        cur.execute("""
            INSERT INTO domain_config
                (ldap_host, domain_suffix, base_dn, admin_group_dn, ssh_host, ssh_user, ssh_pass, use_ldaps, is_active)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,TRUE)
        """, (
            cfg['ldap_host'], cfg['domain_suffix'], cfg['base_dn'],
            cfg['admin_group_dn'], cfg['ssh_host'], cfg['ssh_user'], cfg['ssh_pass'], cfg.get('use_ldaps', False)
        ))
        conn.commit()
        return True
    except Exception as e:
...
def set_domain_use_ldaps(domain_id, value: bool):
    """Cập nhật cờ use_ldaps cho 1 domain đã tồn tại — gọi khi login() phát hiện domain cần
    LDAPS nhưng DB đang lưu sai (vd domain connect từ trước khi có tính năng tự nhận diện)."""
    conn = get_db_connection()
    cur  = conn.cursor()
    try:
        cur.execute("UPDATE domain_config SET use_ldaps=%s WHERE id=%s", (value, domain_id))
        conn.commit()
    except Exception as e:
        conn.rollback()
        print(f"set_domain_use_ldaps error: {e}")
    finally:
        cur.close()
        conn.close()
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if not domain_suffix:
                ...
                # Bước 2: bind thật bằng tài khoản admin để xác thực + lấy quyền — tự động thử
                # LDAP thường (389) trước, fallback sang LDAPS (636) nếu DC bắt buộc signing/
                # channel binding (mặc định trên Windows Server 2025/2026 và các bản mới).
                user_dn = f"{admin_user}{domain_suffix}"
                conn, used_server, used_ldaps, bind_error = smart_ldap_bind(ldap_host, user_dn, admin_pass)
                ...
                if conn:
                    ...
                    if not ssh_ok:
                        ...
                        new_cfg = {
                            ...
                            'ssh_user': admin_user,
                            'ssh_pass': admin_pass,
                            'use_ldaps': used_ldaps,
                        }
                        if save_domain_config(new_cfg):
                            write_log(admin_user, 'CONNECT_DOMAIN',
                                      target=ldap_host, detail=f"domain_suffix={domain_suffix}, base_dn={base_dn}, use_ldaps={used_ldaps} (auto-detected)")
                            return redirect(url_for('login'))
                        else:
                            error = "Lưu cấu hình domain thất bại (DB error)."
                else:
                    error = bind_error or "Không thể đăng nhập với tài khoản đã nhập. Vui lòng kiểm tra lại username/password."
        except Exception as e:
            err_str = str(e).lower()
...
# ─────────────────────────────────────────────
#  LDAP bind thông minh — tự nhận diện DC nào bắt buộc LDAPS (636, mã hoá) thay vì LDAP
#  thường (389, không mã hoá) và TỰ ĐỘNG fallback, không cần người dùng phải biết trước.
#
#  Từ khoảng Windows Server 2022 trở đi (và BẮT BUỘC mặc định trên các bản mới như 2025/2026),
#  2 policy "LDAP server signing requirements" và "LDAP server channel binding token
#  requirements" mặc định = Require/Always. Khi đó DC từ chối THẲNG mọi simple bind không mã
#  hoá qua cổng 389 — kể cả đúng 100% username/password — với lỗi kèm cụm từ đặc trưng "the
#  server requires binds to turn on integrity checking". Rất dễ nhầm là sai mật khẩu.
# ─────────────────────────────────────────────
_LDAPS_REQUIRED_HINTS = (
    'integrity checking',        # thông báo gốc của AD khi từ chối simple bind không mã hoá
    'confidentiality required',
    'strongerauthrequired',
)

def _looks_like_ldaps_required(exc_or_msg) -> bool:
    """Nhận diện đúng lỗi 'DC bắt buộc LDAP signing/channel binding' từ message trả về của
    ldap3/AD, để KHÔNG fallback nhầm khi lỗi thật sự là do sai mật khẩu/tài khoản khoá."""
    text = str(exc_or_msg).lower()
    return any(hint in text for hint in _LDAPS_REQUIRED_HINTS)

def _build_ldaps_server(host, get_info=None, connect_timeout=5):
    """Server LDAPS (636, TLS). validate=ssl.CERT_NONE vì phần lớn DC nội bộ dùng chứng chỉ
    self-signed hoặc AD CS nội bộ, không có CA công cộng để xác thực chuỗi chứng chỉ — vẫn mã
    hoá đường truyền đầy đủ, chỉ bỏ qua bước kiểm tra chuỗi cert (phù hợp mạng nội bộ)."""
    tls = Tls(validate=ssl.CERT_NONE)
    return Server(host, port=636, use_ssl=True, tls=tls, get_info=get_info, connect_timeout=connect_timeout)

def smart_ldap_bind(host, user_dn, password, get_info=None, connect_timeout=5, prefer_ldaps=False):
    """Bind LDAP, tự động xử lý cả 2 kiểu DC:
    - DC cũ (2019/2022 mặc định): LDAP thường qua 389 — vẫn hoạt động bình thường, không đổi gì.
    - DC mới (2025/2026, hoặc DC cũ đã tự bật signing bắt buộc): từ chối simple bind không mã
      hoá dù ĐÚNG mật khẩu. Khi gặp đúng lỗi này, TỰ ĐỘNG thử lại qua LDAPS (636, TLS).

    prefer_ldaps=True: bỏ qua bước thử LDAP thường, vào thẳng LDAPS luôn — dùng cho domain đã
    từng xác định là cần LDAPS (cfg['use_ldaps']=True), tránh mất thời gian thử lại mỗi lần.

    Trả về tuple (conn, server, used_ldaps, error_message):
      conn=None nếu cả 2 cách đều thất bại (kèm error_message dễ hiểu cho người dùng).
    """
    if not prefer_ldaps:
        try:
            server = Server(host, get_info=get_info, connect_timeout=connect_timeout)
            conn   = Connection(server, user=user_dn, password=password, authentication='SIMPLE')
            if conn.bind():
                return conn, server, False, None
            # bind() trả về False (ldap3 mặc định không raise exception cho lỗi xác thực) —
            # kiểm tra message lỗi cụ thể để phân biệt "bắt buộc LDAPS" với sai mật khẩu thật.
            bind_msg = f"{conn.result.get('description','')} {conn.result.get('message','')}"
            if not _looks_like_ldaps_required(bind_msg):
                return None, server, False, "Tài khoản hoặc mật khẩu không chính xác."
            # Rơi xuống thử LDAPS bên dưới
        except Exception as e:
            if not _looks_like_ldaps_required(e):
                raise
            # Rơi xuống thử LDAPS bên dưới

    try:
        server = _build_ldaps_server(host, get_info=get_info, connect_timeout=connect_timeout)
        conn   = Connection(server, user=user_dn, password=password, authentication='SIMPLE')
        if conn.bind():
            return conn, server, True, None
        return None, server, True, "Tài khoản hoặc mật khẩu không chính xác."
    except Exception as e:
        err_text = str(e).lower()
        # "wrong version number" / connection reset ngay trong lúc bắt tay TLS (không phải do
        # sai IP/timeout mạng) hầu như luôn là do Domain Controller CHƯA CÓ chứng chỉ hợp lệ gắn
        # cho dịch vụ LDAPS (636) — cổng có mở (TCP connect được) nhưng không có gì để bắt tay
        # TLS. Đây là lỗi PHÍA DC, không sửa được từ app — cần lên DC kiểm tra/cài chứng chỉ.
        if 'wrong version number' in err_text or 'unexpected eof' in err_text or 'connection reset' in err_text or 'ssl' in err_text:
            return None, None, True, (
                f"Cổng 636 (LDAPS) trên '{host}' có mở nhưng bắt tay TLS thất bại — Domain Controller "
                f"CHƯA CÓ chứng chỉ hợp lệ gắn cho dịch vụ LDAPS. Đây là lỗi cần xử lý TRÊN DC, không "
                f"phải do app: kiểm tra certlm.msc trên DC có chứng chỉ ở mục Personal với Server "
                f"Authentication + private key khớp FQDN của DC không; nếu có sẵn AD CS nội bộ, kiểm tra "
                f"autoenrollment (gpupdate /force) rồi restart dịch vụ AD DS hoặc restart DC."
            )
        return None, None, True, f"Không kết nối được LDAPS (636) tới '{host}': {e}"
...
def login():
    ...
    if request.method == 'POST':
        if not cfg:
            ...
            user_dn  = f"{username}{cfg['domain_suffix']}"
            try:
                # prefer_ldaps=True nếu domain này đã từng xác định cần LDAPS (lưu lúc Connect
                # Domain) — vào thẳng LDAPS (636), khỏi mất thời gian thử LDAP thường (389) trước
                # rồi mới fallback mỗi lần đăng nhập.
                conn, used_server, used_ldaps, bind_error = smart_ldap_bind(
                    cfg['ldap_host'], user_dn, password,
                    connect_timeout=3, prefer_ldaps=bool(cfg.get('use_ldaps'))
                )
                ...
                if conn:
                    ...
                    # Nếu lần này phát hiện domain cần LDAPS mà DB đang lưu use_ldaps=False (vd
                    # domain được connect từ trước khi có cơ chế tự nhận diện này) -> cập nhật lại
                    # để các lần đăng nhập SAU của user khác vào thẳng LDAPS luôn, không phải chờ
                    # thử LDAP thường thất bại trước.
                    if used_ldaps and not cfg.get('use_ldaps'):
                        set_domain_use_ldaps(cfg['id'], True)
                    ...
                    write_log(username, 'LOGIN', detail=f"is_admin={is_admin}, domain={cfg['ldap_host']}, use_ldaps={used_ldaps}")
                    ...
                    error = bind_error or "Tài khoản hoặc mật khẩu AD không chính xác."
            except Exception as e:
